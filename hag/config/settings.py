...
class HassOptions(BaseModel):
    ...
    state_check_interval: int = Field(
        default=300000, description="State check interval in milliseconds"
    )
...
class TemperatureThresholds(BaseModel):
    ...
    outdoor_min: float = Field(
        ..., description="Minimum outdoor temperature for operation"
    )
    outdoor_max: float = Field(
        ..., description="Maximum outdoor temperature for operation"
    )
...
class DefrostOptions(BaseModel):
    ...
    temperature_threshold: float = Field(
        default=0.0, description="Temperature below which defrost is needed"
    )
    period_seconds: int = Field(
        default=3600, description="Defrost cycle period in seconds"
    )
    duration_seconds: int = Field(
        default=300, description="Defrost cycle duration in seconds"
    )
...
class HvacOptions(BaseModel):
    ...
    outdoor_sensor: str = Field(
        default="sensor.openweathermap_temperature",
        description="Outdoor temperature sensor",
    )
    system_mode: SystemMode = Field(
        default=SystemMode.AUTO, description="System operation mode"
    )
    hvac_entities: List[HvacEntity] = Field(
        default_factory=list, description="HVAC entities to control"
    )
...
class ApplicationOptions(BaseModel):
    ...
    use_ai: bool = Field(
        default=False, description="Enable AI agent for HVAC decisions"
    )
